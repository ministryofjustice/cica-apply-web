...
window.CICA = {
    SERVICE_URL: process.env.CW_URL
};
jest.mock('js-cookie');
...
describe('GA', () => {
    ...
    describe('Internal link', () => {
        it('should not send an event to GA', () => {
            document.body.innerHTML = `
                <a href="${process.env.CW_URL}/some-page-url" id="internal-test-1">Click here</a>
            `;
            const cicaGa = createCicaGa(window);
            cicaGa.init();
            window.gtag = jest.fn();
    ...
    describe('Download link', () => {
        it('should send an event to GA', () => {
            document.body.innerHTML = `
                <a href="${process.env.CW_URL}/stuff/stuff.zip" id="download-test-1">Click here</a>
            `;
            const cicaGa = createCicaGa(window);
            cicaGa.init();
            window.gtag = jest.fn();
            ...
            expect(window.gtag).toHaveBeenCalledWith('event', 'click', {
                ...
                event_label: `${process.env.CW_URL}/stuff/stuff.zip`,
                event_callback: expect.any(Function)
            });
        });
    });
