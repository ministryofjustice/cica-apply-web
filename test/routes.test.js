...
describe('Data capture service endpoints', () => {
    describe('Cica-web static routes', () => {
        ...
        describe('/new-claim', () => {
            describe('GET', () => {
                describe('200', () => {
                    it('Should respond with a 200 status and render the new claim page', async () => {
                        const response = await request(app).get('/new-claim');
                        expect(response.statusCode).toBe(200);
                    });
                    it('Should render a page with the new claim page heading', async () => {
                        const response = await request(app).get('/new-claim');
                        const actual = response.res.text.replace(/\s+/g, '');
                        const pageHeading = `<h1 class="govuk-heading-xl">New way to claim compensation</h1>`.replace(
                            /\s+/g,
                            ''
                        );
                        expect(actual).toContain(pageHeading);
