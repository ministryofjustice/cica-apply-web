...
(function (root, factory) {
  ...
  MOJFrontend.initAll = function (options) {
    ...
    options = typeof options !== 'undefined' ? options : {}; // Allow the user to initialise MOJ Frontend in only certain sections of the page
...
/* harmony import */ var _utils_formHasAllBlankAnswers__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./utils/formHasAllBlankAnswers */ "./src/modules/ga/utils/formHasAllBlankAnswers/index.js");
function ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); if (enumerableOnly) symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; }); keys.push.apply(keys, symbols); } return keys; }
        ...
        click(element, {
          ...
          if (element.classList.contains('ga-event--sign-in')) {
            send({
              ...
              category: 'application',
              label: (!Object(_utils_formHasAllBlankAnswers__WEBPACK_IMPORTED_MODULE_2__["default"])(window.document.querySelector('form'))).toString()
            });
          }
        });
        return;
...
/***/ "./src/modules/ga/utils/formHasAllBlankAnswers/index.js":
/*!**************************************************************!*\
  !*** ./src/modules/ga/utils/formHasAllBlankAnswers/index.js ***!
  \**************************************************************/
/*! exports provided: default */
...
__webpack_require__.r(__webpack_exports__);
/* globals FormData */
var keysToIngore = ['_csrf'];

function formHasAllBlankAnswers(form) {
  var formData = new FormData(form);
  var answeredQuestions = Array.from(formData.entries()).filter(entry => !keysToIngore.includes(entry[0])).filter(entry => !!entry[1]);
  return answeredQuestions.length === 0;
}

/* harmony default export */ __webpack_exports__["default"] = (formHasAllBlankAnswers);
...
/* harmony default export */ __webpack_exports__["default"] = (msToMinutesAndSeconds);
...
function createPostcodeLookup(window) {
  ...
  var addressSearchInput; // Matches a single letter (e.g 'A'), number range (e.g. 110-114), delimiter (e.g. 1/2)
  ...
  function mapSelectionToAddressFormInputs() {
    ...
    clearAddressForm(); // The following is based on the rules for generating multi-line addresses which are
    ...
    var dpaCounty = result.LOCAL_CUSTODIAN_CODE_DESCRIPTION === 'ORDNANCE SURVEY' ? '' : result.LOCAL_CUSTODIAN_CODE_DESCRIPTION; // Add a "PO BOX " prefix to the PO Box Number integer.
    ...
    var addressLines = []; // Does the page contain an organisation name?

    var organisationName = window.document.querySelector('[id *= "organisation-name"]');
    ...
    addressLines.push(dpaPOBoxNumber, dpaOrganisationName); // Merge the structured premises and thoroughfare components into the address array.

    addressLines = addressLines.concat(premises);
    addressLines = addressLines.concat(premisesThoroughfareLocality);
    addressLines = addressLines.concat(thoroughfareLocality); // Remove any duplicates and blanks from the address array.
    ...
    var addressLine1 = window.document.querySelector('[id *= "street"]');
    addressLine1.value = selectionLine1;
    var addressLine2 = window.document.querySelector('[id *= "street-2"]');
    addressLine2.value = selectionLine2 || '';
    var addressLine3 = window.document.querySelector('[id *= "street-3"]');
    addressLine3.value = selectionLine3 || '';
    var addressTown = window.document.querySelector('[id *= "town-or-city"]');
    addressTown.value = dpaPostTown;
    var addressCounty = window.document.querySelector('[id *= "county"]');
    addressCounty.value = dpaCounty;
    var addressPostcode = window.document.querySelector('[id *= "postcode"]');
  ...
  function addSearchResultsToSelectElement(data) {
    clearAddressResultsDropdown(); // Update the JSON object with the data results.
    ...
    selectElementResults.add(option); // See also {@link https://apidocs.os.uk/reference/os-places-dpa-output}

    var dataset = 'DPA'; // Loop through the data results; adding the address string as a new option
  ...
  function displayFieldErrors(message) {
    ...
    errorOuterSpan.appendChild(errorInnerSpan); // const addressSearchInput = window.document.getElementById('address-search-input');
  ...
  function removeErrorMessages() {
    var pageTitle = window.document.title.replace('Error: ', ''); // eslint-disable-next-line no-param-reassign
  ...
  function displayErrors(message) {
    ...
    var pageErrorTitle = "Error: ".concat(window.document.title); // eslint-disable-next-line no-param-reassign
  ...
  function createContentElements() {
    ...
    descriptionDiv.appendChild(window.document.createTextNode('Enter a UK postcode to search for and select an address. Or, you can enter the address manually.'));
  ...
  function createPostcodeSearchElements() {
    var addressSearchLabelContent = window.document.createTextNode('Postcode');
...
/* harmony default export */ __webpack_exports__["default"] = (createPostcodeLookup);
