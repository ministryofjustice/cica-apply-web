...
describe('Answer helper', () => {
    describe('Dynamic answer formatter', () => {
        ...
        it('should format a question with more than one but less than 4 answers on a single line', () => {
            expect(formattedAnswer.p_applicant_enter_your_name.value).toMatch('Mr Barry Piccinni');
        });

        it('should format a question with 4 or more answers appears in a multi-line format', () => {
            expect(formattedAnswer.p_applicant_enter_your_address.value).toMatch(
                'Alexander Bain House<br>Atlantic Quay<br>Glasgow<br>G2 8JQ'
            );
        });

        it('should correctly build the url for each question', () => {
            expect(formattedAnswer.p_applicant_enter_your_address.href).toMatch(
                '/apply/applicant-enter-your-address?next=check-your-answers'
            );
            expect(formattedAnswer.p_applicant_enter_your_name.href).toMatch(
                '/apply/applicant-enter-your-name?next=check-your-answers'
            );
            expect(
                formattedAnswer.p_applicant_were_you_a_victim_of_sexual_assault_or_abuse.href
            ).toMatch(
    ...
    describe('questionNameFormatter', () => {
        it('should replace all "-" with "_" in a string', () => {
            const inputString = 'i-am-a-section';

            const actual = answerFormatHelper.questionNameFormatter(inputString);

            expect(actual).toMatch('i_am_a_section');
        });
    });
    describe('arrayFormatter', () => {
        it('should return all the elements of an array on a single, comma separated line', () => {
            ...
            expect(actual).toMatch('i-am-an-answer, another-answer, a-third-answer');
        });
    });
});
