...
function createDashboardService(ownerId) {
    ...
    async function getActionData() {
        ...
        const templateData = dataset.reduce((acc, questionnaireData) => {
            const resumeLink = questionnaireData.analyticsId
                ? `/actions/${questionnaireData.type}/resume/${questionnaireData.questionnaireId}?a_id=${questionnaireData.analyticsId}`
                : `/actions/${questionnaireData.type}/resume/${questionnaireData.questionnaireId}`;
            acc.push([
                {
